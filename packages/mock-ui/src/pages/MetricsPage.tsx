...
export function MetricsPage({ t }: MetricsPageProps) {
  ...
  const [tab, setTab] = useState<'summary' | 'health' | 'prometheus'>(
    'summary',
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [s, h] = await Promise.all([
        mockApi.metrics.summary(),
        mockApi.metrics.health(),
      ]);
      setSummary(s);
      setHealth(h);
    } catch (e) {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              {T('metrics.title', 'Metrics')}
            </h1>
            <p className="text-text-secondary text-sm">
              {T(
                'metrics.subtitle',
                'Prometheus counters, histograms and service health',
              )}
            </p>
          </div>
        </div>
        <button
          ...
          title={T('metrics.refresh', 'Refresh')}
        >
          <RefreshCw size={15} />
        </button>
      </div>
      ...
      <div className="flex gap-1 mb-6 bg-white/5 rounded-lg p-1 w-fit">
        {(['summary', 'health', 'prometheus'] as const).map((tabKey) => (
          ...
          >
            {(() => {
              const [k, fb] = TAB_LABELS[tabKey] ?? [tabKey, tabKey];
              return T(k, fb);
            })()}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(summary.counters).map(([k, v]) => (
                ...
                >
                  ...
                  <span className="text-lg font-bold text-orange-400 shrink-0 ml-2">
                    {v}
                  </span>
                </div>
              ))}
          ...
          {Object.keys(summary.histograms).length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-text-secondary mb-3 uppercase tracking-wide">
                {T('metrics.histograms', 'Histograms')}
              </h3>
        ...
        prometheus ? (
          ...
          <div className="text-center text-text-secondary py-12">
            {T('metrics.loadingPrometheus', 'Loading Prometheus text…')}
          </div>
        )
      ) : null}
    </div>
