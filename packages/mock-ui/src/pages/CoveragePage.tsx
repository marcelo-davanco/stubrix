...
export function CoveragePage({ t }: CoveragePageProps) {
  ...
  const [specFileName, setSpecFileName] = useState<string | undefined>(
    undefined,
  );
  ...
  function isPostmanCollection(content: string): boolean {
    try {
      ...
      try {
        const schemaUrl = new URL(String(info['schema'] ?? ''));
        return (
          schemaUrl.hostname === 'getpostman.com' ||
          schemaUrl.hostname.endsWith('.getpostman.com')
        );
      } catch {
        return false;
      }
  ...
  async function analyze() {
    ...
    try {
      if (tab === 'url' && specUrl) {
        ...
        if (!fetchRes.ok)
          throw new Error(
            `Failed to fetch spec: ${fetchRes.status} ${fetchRes.statusText}`,
          );
        ...
        const res = await mockApi.coverage.analyzePostman(
          content,
          specFileName,
        );
        setReport(res);
      ...
      <div className="flex items-center gap-3 mb-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            ...
            {T('coverage.title', 'Mock Coverage')}
          </h1>
          <p className="text-text-secondary text-sm">
            {T(
              'coverage.subtitle',
              'Analyze how well your mocks cover an OpenAPI spec or Postman collection',
            )}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-lg p-5 mb-6">
        <div className="flex gap-1 mb-4 bg-white/5 rounded-lg p-1 w-fit">
          {(['paste', 'url'] as const).map((tabKey) => (
            ...
            >
              {tabKey === 'paste'
                ? T('coverage.pasteUpload', 'Paste / Upload')
                : T('coverage.fromUrl', 'From URL')}
            </button>
          ))}
        </div>
        ...
        {tab === 'paste' && (
          <>
            <div className="flex items-center gap-3 mb-3">
              ...
              {specContent && (
                <span className="text-xs text-green-400">
                  {Tvars(
                    'coverage.fileLoaded',
                    `File loaded (${specContent.length} chars)`,
                    { count: specContent.length },
                  )}
                  {isPostmanCollection(specContent)
                    ? T(
                        'coverage.postmanDetected',
            ...
            <textarea
              ...
              rows={10}
              placeholder={T(
                'coverage.specPlaceholder',
                'Paste your OpenAPI spec (YAML or JSON) or Postman collection (JSON)…',
              )}
        ...
        >
          <BarChart2 size={13} />{' '}
          {loading
            ? T('coverage.analyzing', 'Analyzing…')
            : T('coverage.analyze', 'Analyze Coverage')}
        </button>
      </div>
        ...
        <>
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white/5 border border-white/10 rounded-lg p-4 text-center">
              ...
              <div className="text-text-secondary text-sm mt-1">
                {T('coverage.covered', 'Covered')}
              </div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold">{report.totalEndpoints}</div>
              <div className="text-text-secondary text-sm mt-1">
                {T('coverage.totalEndpoints', 'Total endpoints')}
              </div>
            </div>
          </div>
                ...
                <thead className="border-b border-white/10">
                  <tr>
                    <th className="text-left px-4 py-2 text-text-secondary font-medium">
                      {T('coverage.method', 'Method')}
                    </th>
                    <th className="text-left px-4 py-2 text-text-secondary font-medium">
                      {T('coverage.path', 'Path')}
                    </th>
                    <th className="text-left px-4 py-2 text-text-secondary font-medium">
                      {T('coverage.status', 'Status')}
                    </th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <td className="px-4 py-2">
                        {entry.status === 'covered' ? (
                          <span className="flex items-center gap-1 text-green-400 text-xs">
                            <CheckCircle size={12} />{' '}
                            {T('coverage.coveredStatus', 'covered')}
                          </span>
                        ) : entry.status === 'partial' ? (
                          <span className="flex items-center gap-1 text-yellow-400 text-xs">
                            <AlertCircle size={12} />{' '}
                            {T('coverage.partialStatus', 'partial')}
                          </span>
                        ) : (
                          <span className="flex items-center gap-1 text-red-400 text-xs">
                            <XCircle size={12} />{' '}
                            {T('coverage.missingStatus', 'missing')}
                          </span>
                        )}
                      </td>
                    </tr>
          ...
          {textReport && (
            <div className="bg-white/5 border border-white/10 rounded-lg p-4">
              <h3 className="text-sm font-medium mb-2">
                {T('coverage.textReport', 'Text Report')}
              </h3>
