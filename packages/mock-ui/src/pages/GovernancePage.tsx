...
export function GovernancePage({ onNavigateBack }: GovernancePageProps) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <ShieldCheck size={22} className="text-blue-400" /> Governance & Linting
          </h1>
          <p className="text-text-secondary text-sm">Validate OpenAPI specs with Spectral rules</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 mb-6 bg-white/5 rounded-lg p-1 w-fit">
        {(['lint', 'rules'] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={[
              'px-4 py-1.5 rounded-md text-sm capitalize transition-colors',
              tab === t ? 'bg-blue-400/20 text-blue-400' : 'text-text-secondary hover:text-text-primary',
            ].join(' ')}
          >
            {t === 'lint' ? 'Lint Spec' : 'Active Rules'}
          </button>
        ))}
      </div>
        ...
        <>
          <div className="bg-white/5 border border-white/10 rounded-lg p-5 mb-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              {specContent && <span className="text-xs text-blue-400">Spec loaded ({specContent.length} chars)</span>}
            </div>
            <textarea
              value={specContent}
            ...
            >
              <ShieldCheck size={13} /> {loading ? 'Linting…' : 'Lint Spec'}
            </button>
          </div>
            ...
            <div className="space-y-4">
              <div className={`flex items-center gap-3 p-4 rounded-lg border ${result.valid ? 'border-green-500/20 bg-green-500/10' : 'border-red-500/20 bg-red-500/10'}`}>
                ...
                <span className={`font-medium ${result.valid ? 'text-green-300' : 'text-red-300'}`}>
                  {result.valid ? 'Spec is valid — no issues found' : `${errors.length} error(s), ${warnings.length} warning(s)`}
                </span>
              </div>
      ...
      {tab === 'rules' && (
        <div className="bg-white/5 border border-white/10 rounded-lg overflow-hidden">
          {loadingRules ? (
            <div className="text-center text-text-secondary py-12">Loading rules…</div>
          ) : rules.length === 0 ? (
            <div className="text-center text-text-secondary py-12">No rules found</div>
          ) : (
            rules.map((rule, i) => (
