    ...
    <div className="p-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              {T('tracing.title', 'Distributed Tracing')}
            </h1>
            <p className="text-text-secondary text-sm">
              {T('tracing.subtitle', 'OpenTelemetry traces via Jaeger')}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {jaegerAvailable !== null && (
            <span
              className={`text-xs px-2 py-1 rounded-full ${jaegerAvailable ? 'bg-green-400/10 text-green-400' : 'bg-red-400/10 text-red-400'}`}
            >
              {jaegerAvailable
                ? T('tracing.jaegerConnected', '● Jaeger connected')
                : T('tracing.jaegerUnavailable', '● Jaeger unavailable')}
            </span>
          )}
          <button
            onClick={load}
      ...
      {loading ? (
        <div className="text-center text-text-secondary py-12">
          {T('tracing.loading', 'Loading…')}
        </div>
      ) : traces.length === 0 ? (
        <EmptyState
          message={T(
            'tracing.empty',
            'No traces found. Make sure Jaeger is running and requests are being traced.',
          )}
        />
      ) : (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {trace.services.map((s) => (
                      <span
                        key={s}
                        className="text-xs bg-cyan-400/10 text-cyan-400 px-1.5 py-0.5 rounded"
                      >
                        {s}
                      </span>
                    ))}
                ...
                <div className="flex items-center gap-4 shrink-0">
                  <span className="text-xs text-text-secondary">
                    {trace.spans.length} {T('tracing.spans', 'spans')}
                  </span>
                  <span className="text-xs text-text-secondary">
                    {trace.duration}ms
                  </span>
                  ...
                  <div className="space-y-2">
                    {trace.spans.map((span, i) => (
                      <div key={i} className="flex items-center gap-3 text-sm">
                        ...
                        <span className="font-mono text-xs text-text-secondary w-24 shrink-0">
                          {span.service}
                        </span>
