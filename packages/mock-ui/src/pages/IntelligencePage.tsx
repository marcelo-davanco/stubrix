...
export function IntelligencePage({ t }: IntelligencePageProps) {
  ...
  async function indexMocks() {
    ...
    try {
      ...
      setSuccess(
        Tvars(
          'intelligence.indexedSuccess',
          `Indexed ${res.indexed} mocks into the knowledge base`,
          { count: res.indexed },
        ),
      );
    } catch (e) {
  ...
  const tabConfig: Array<{
    ...
    {
      id: 'query',
      labelKey: 'intelligence.ragQuery',
      label: 'RAG Query',
      icon: <Search size={13} />,
    },
    {
      id: 'mock',
      labelKey: 'intelligence.suggestMock',
      label: 'Suggest Mock',
      icon: <Sparkles size={13} />,
    },
    {
      id: 'data',
      labelKey: 'intelligence.suggestData',
      label: 'Suggest Data',
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              {T('intelligence.title', 'Intelligence')}
            </h1>
            <p className="text-text-secondary text-sm">
              {T(
                'intelligence.subtitle',
                'AI-powered mock generation and knowledge queries',
              )}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {available !== null && (
            <span
              className={`text-xs px-2 py-1 rounded-full ${available ? 'bg-green-400/10 text-green-400' : 'bg-red-400/10 text-red-400'}`}
            >
              {available
                ? T('intelligence.openRagConnected', '● OpenRAG connected')
                : T('intelligence.openRagUnavailable', '● OpenRAG unavailable')}
            </span>
          )}
          <button
          ...
          >
            ...
            {indexing
              ? T('intelligence.indexing', 'Indexing…')
              : T('intelligence.reindexMocks', 'Re-index Mocks')}
          </button>
        </div>
      </div>
      ...
      {!available && (
        <div className="flex items-start gap-3 rounded-xl border border-yellow-500/20 bg-yellow-500/10 p-4 mb-6 text-sm text-yellow-300">
          ...
          <span>
            {T(
              'intelligence.notConfigured',
              'OpenRAG is not configured. Set the OPENAI_API_KEY or RAG_ENDPOINT env variable to enable AI features.',
            )}
          </span>
        </div>
      )}
        ...
        <div>
          <div className="flex gap-2 mb-4">
            <input
              ...
              placeholder={T(
                'intelligence.queryPlaceholder',
                'Ask anything about your mocks, schemas or docs…',
              )}
              ...
              {queryResult.sources.length > 0 && (
                <div>
                  <p className="text-xs text-text-secondary mb-1">
                    {T('intelligence.sources', 'Sources:')}
                  </p>
                  <div className="flex flex-wrap gap-1">
                    {queryResult.sources.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-purple-400/10 text-purple-300 px-2 py-0.5 rounded"
                      >
                        {s}
                      </span>
                    ))}
      ...
      {tab === 'mock' && (
        <div>
          <textarea
            ...
            rows={4}
            placeholder={T(
              'intelligence.mockDescPlaceholder',
              "Describe the mock you want… e.g. 'A GET /api/products endpoint that returns a list of 3 products with id, name and price'",
            )}
          ...
          {mockResult && (
            <div className="bg-white/5 border border-white/10 rounded-lg p-4 space-y-3">
              ...
              <pre className="text-xs font-mono overflow-auto max-h-64 bg-white/5 rounded p-3">
                {JSON.stringify(mockResult.mapping, null, 2)}
              </pre>
            </div>
          )}
        </div>
      ...
      {tab === 'data' && (
        <div>
          <textarea
            ...
            rows={4}
            placeholder={T(
              'intelligence.dataDescPlaceholder',
              "Describe the seed data you want… e.g. 'Insert 5 users with name, email and role (admin or viewer)'",
            )}
            className="w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-sm focus:outline-none focus:border-purple-400 resize-none mb-3"
          />
          <button
          ...
          {dataResult && (
            <div className="bg-white/5 border border-white/10 rounded-lg p-4 space-y-3">
              <p className="text-sm text-text-secondary">
                {dataResult.explanation}
              </p>
              <pre className="text-xs font-mono overflow-auto max-h-64 bg-white/5 rounded p-3 text-green-300">
                {dataResult.sql}
              </pre>
            </div>
          )}
