...
export class ContractsService {
  ...
  constructor(private readonly config: ConfigService) {
    this.brokerUrl =
      this.config.get<string>('PACT_BROKER_URL') ?? 'http://localhost:9292';
  }
  ...
  async listContracts(): Promise<PactContract[]> {
    try {
      ...
      const data = (await res.json()) as {
        _embedded?: {
          pacts?: Array<{
            _links: { self: Array<{ href: string; name: string }> };
            _embedded: {
  ...
  async canIDeploy(
    ...
    try {
      ...
      const data = (await res.json()) as {
        summary?: { deployable?: boolean; reason?: string };
      };
      ...
      return {
        ...
        message:
          data.summary?.reason ??
          (deployable ? 'All verifications passed' : 'Verification failed'),
      };
    } catch (err) {
      return {
        success: false,
  ...
  async healthCheck(): Promise<{ available: boolean; url: string }> {
    try {
      const res = await fetch(`${this.brokerUrl}`, {
