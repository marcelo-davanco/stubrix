...
export class CoverageService {
  ...
  async analyze(specContent: string, specFile?: string): Promise<CoverageReport> {
  ...
  generateTextReport(report: CoverageReport): string {
    ...
    lines.push('╚══════════════════════════════════════════╝');
    lines.push('');
    lines.push(`  Coverage: ${bar(report.coveragePercent)} ${report.coveragePercent}%`);
    lines.push('');
    ...
    if (report.entries.length > 0) {
      ...
      for (const e of report.entries) {
        const icon = e.status === 'covered' ? '✅' : e.status === 'partial' ? '⚠️ ' : '❌';
        lines.push(`  ${icon} ${e.method.padEnd(7)} ${e.path}`);
        if (e.missingCodes.length > 0) {
          lines.push(`         Missing status codes: ${e.missingCodes.join(', ')}`);
        }
      }
    }
  ...
  private matchEndpoint(spec: SpecEndpoint, mocks: MockEndpoint[]): CoverageEntry {
    const matching = mocks.filter((m) =>
      m.method === spec.method && this.pathMatches(spec.path, m),
    );

    const mockedCodes = [...new Set(matching.flatMap((m) => m.responseCodes))];
    const missingCodes = spec.responseCodes.filter((c) => !mockedCodes.includes(c));
