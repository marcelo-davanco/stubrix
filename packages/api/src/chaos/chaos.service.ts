...
export class ChaosService {
    ...
    return fs
      ...
      .flatMap((f) => {
        try {
          return [
            JSON.parse(
              fs.readFileSync(path.join(this.profilesDir, f), 'utf-8'),
            ) as FaultProfile,
          ];
        } catch {
          return [];
  ...
  createProfile(
    ...
    // codeql[js/http-to-file-access] - intentional: API request body (chaos profile definition) is persisted to storage
    fs.writeFileSync(
  ...
  evaluate(urlPath: string, method: string): ChaosResult {
    ...
    for (const profile of active) {
      ...
      if (
        ...
        !profile.methods.includes(method.toUpperCase())
      )
        continue;
