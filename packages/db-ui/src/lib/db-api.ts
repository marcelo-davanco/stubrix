...
export const dbApi = {
  ...
  getProjectDatabaseConfigs: (projectId: string) =>
    request<Array<ProjectDatabaseConfigItem>>(
      `${API_BASE}/projects/${encodeURIComponent(projectId)}/databases/configs`,
    ),
  upsertProjectDatabaseConfig: (
    projectId: string,
    payload: UpsertProjectDatabaseConfigPayload,
  ) =>
    request<ProjectDatabaseConfigItem>(
      `${API_BASE}/projects/${encodeURIComponent(projectId)}/databases/configs`,
      {
        method: 'POST',
  ...
  toggleProjectDatabaseConfig: (projectId: string, id: string) =>
    request<ProjectDatabaseConfigItem>(
      `${API_BASE}/projects/${encodeURIComponent(projectId)}/databases/configs/${encodeURIComponent(id)}/toggle`,
      {
        method: 'PATCH',
      },
    ),
  deleteProjectDatabaseConfig: (projectId: string, id: string) =>
    request<MessageResponse>(
      `${API_BASE}/projects/${encodeURIComponent(projectId)}/databases/configs/${encodeURIComponent(id)}`,
      {
        method: 'DELETE',
      },
    ),
  testProjectDatabaseConfig: (projectId: string, id: string) =>
    request<{ ok: boolean; message: string }>(
      `${API_BASE}/projects/${encodeURIComponent(projectId)}/databases/configs/${encodeURIComponent(id)}/test`,
    ),
  ...
  getDatabases: (engine?: string, projectId?: string, connectionId?: string) =>
    request<DatabasesResponse>(
      withQueryParams(
        engine
          ? `${DB_API_BASE}/engines/${engine}/databases`
          : `${DB_API_BASE}/databases`,
  ...
  getDatabaseInfo: (
    ...
    request<DatabaseInfo>(
      withQueryParams(
        engine
          ? `${DB_API_BASE}/engines/${engine}/databases/${name}/info`
          : `${DB_API_BASE}/databases/${name}/info`,
        { projectId, connectionId },
      ),
    ),
  getSnapshots: (projectId?: string) =>
    request<SnapshotsResponse>(
      projectId
        ? `${DB_API_BASE}/snapshots?projectId=${encodeURIComponent(projectId)}`
        : `${DB_API_BASE}/snapshots`,
    ),
  createSnapshot: (
    engine: string,
    payload: {
    ...
    request<SnapshotMutationResponse>(
      `${DB_API_BASE}/engines/${engine}/snapshots`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
  ...
  updateSnapshot: (name: string, payload: Record<string, unknown>) =>
    request<SnapshotUpdateResponse>(
      `${DB_API_BASE}/snapshots/${encodeURIComponent(name)}`,
      {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
  ...
  restoreSnapshot: (
    ...
    request<MessageResponse>(
      `${DB_API_BASE}/engines/${engine}/snapshots/${encodeURIComponent(name)}/restore`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
