    ...
    <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-5">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-text-primary">{info.database}</p>
      ...
      {info.tables.length === 0 ? (
        <p className="text-xs text-text-secondary">Nenhuma tabela encontrada.</p>
      ) : (
        <div>
          <div className="mb-2 flex items-center gap-1.5">
            <Table2 size={12} className="text-text-secondary" />
            <p className="text-xs font-medium text-text-secondary">{info.tables.length} tabelas</p>
          </div>
