...
server.tool(
  ...
  {
    projectId: z.string().describe('Project ID'),
    targetUrl: z.string().describe('Target API URL to proxy and record from'),
  },
...
server.tool(
  ...
  async ({ projectId }) => api(`/api/projects/${projectId}/recording/status`),
);
...
server.tool(
  ...
  async ({ projectId }) => api(`/api/projects/${projectId}/databases/configs`),
);
...
server.prompt(
  ...
  'Guides through setting up a complete recording session for API traffic',
  {
    projectId: z
      .string()
      .optional()
      .describe('Optional: Project ID to use (will create if not provided)'),
    proxyTarget: z
      .string()
      .describe(
        'Target API URL to record from (e.g., https://api.example.com)',
      ),
    includePatterns: z
      .string()
      .optional()
      .describe(
        'Optional: Comma-separated URL patterns to include (e.g., /api/*,/api/users/**)',
      ),
    excludePatterns: z
      .string()
      .optional()
      .describe(
        'Optional: Comma-separated URL patterns to exclude (e.g., /api/health,/api/metrics/*)',
      ),
  },
...
server.prompt(
  ...
  'Creates a complete mock from natural language description',
  {
    ...
    description: z
      .string()
      .describe('Natural language description of the mock behavior'),
  },
...
server.prompt(
  ...
  'Guides through snapshot, make changes, and restore database workflow',
  {
    ...
    databaseEngine: z
      .string()
      .optional()
      .describe('Optional: Database engine (postgres, mysql, sqlite)'),
    snapshotName: z
      .string()
      .optional()
      .describe('Optional: Custom snapshot name'),
  },
...
server.prompt(
  ...
  'Comprehensive health check of all Stubrix services and components',
  {
    includeDatabases: z
      .boolean()
      .optional()
      .describe('Optional: Include database health check'),
    includeContainers: z
      .boolean()
      .optional()
      .describe('Optional: Include Docker container health'),
  },
...
server.tool(
  ...
  'Preview what mocks would be imported from content without actually creating them.',
  {
    content: z
      .string()
      .describe(
        'Raw content of the file (HAR JSON, Postman JSON, OpenAPI JSON/YAML)',
      ),
    filename: z
      .string()
      .optional()
      .describe(
        'Original filename to help detect format (e.g., spec.yaml, collection.json)',
      ),
    baseUrl: z
      .string()
      .optional()
      .describe('Base URL for OpenAPI specs without a server block'),
  },
...
server.tool(
  ...
  'Import mocks from a URL (e.g., a public OpenAPI spec URL). Auto-detects format.',
  {
    url: z
      .string()
      .describe(
        'URL of the spec/collection to import (must be publicly accessible)',
      ),
    projectId: z.string().describe('Target project ID'),
    deduplicate: z
      .boolean()
      .optional()
      .describe('Skip duplicate mappings (default: true)'),
    overwrite: z
      .boolean()
      .optional()
      .describe('Overwrite existing mappings (default: false)'),
    filterMethods: z
      .array(z.string())
      .optional()
      .describe("Only import specific HTTP methods, e.g. ['GET', 'POST']"),
    filterStatusCodes: z
      .array(z.number())
      .optional()
      .describe('Only import specific response status codes, e.g. [200, 201]'),
  },
  async ({
    url,
    projectId,
...
server.tool(
  ...
  'Import mocks from raw file content (HAR, Postman Collection v2.1, OpenAPI 3.x, Swagger 2.0). Auto-detects format.',
  {
    ...
    filename: z
      .string()
      .optional()
      .describe('Original filename to help detect format'),
    deduplicate: z
      .boolean()
      .optional()
      .describe('Skip duplicate mappings (default: true)'),
    ...
    filterMethods: z
      .array(z.string())
      .optional()
      .describe('Only import specific HTTP methods'),
    filterStatusCodes: z
      .array(z.number())
      .optional()
      .describe('Only import specific response status codes'),
  },
  async ({
    content,
    projectId,
...
server.tool(
  ...
  async ({ id }) => api(`/api/scenarios/${id}/restore`, { method: 'POST' }),
);
...
server.tool(
  ...
  async ({ id }) => api(`/api/scenarios/${id}`, { method: 'DELETE' }),
);
...
server.tool(
  ...
  'Generate a WireMock mapping suggestion from a natural language endpoint description.',
  {
    description: z
      .string()
      .describe(
        "Natural language description, e.g. 'GET /users returns list of users with 200'",
      ),
  },
  async ({ description }) =>
...
server.tool(
  ...
  'Generate SQL seed data from a natural language description.',
  {
    description: z
      .string()
      .describe(
        "Natural language description, e.g. 'Insert 10 users into users table'",
      ),
  },
  async ({ description }) =>
...
server.tool(
  ...
  {},
  async () => api('/api/intelligence/index', { method: 'POST' }),
);
      ...
      .array(
        z.object({
          type: z
            .string()
            .describe(
              'Fault type: delay | error | timeout | random_error | bandwidth_limit',
            ),
...
server.tool(
  ...
  'Apply a built-in chaos preset as a new fault profile.',
  {
    preset: z
      .string()
      .describe(
        'Preset name: slow-network | flaky-service | total-outage | timeout',
      ),
...
server.tool(
  ...
  async ({ pacticipant, version }) =>
    api(
      `/api/contracts/can-i-deploy?pacticipant=${encodeURIComponent(pacticipant)}&version=${encodeURIComponent(version)}`,
    ),
);
...
server.tool(
  ...
  'Apply a network chaos preset to a proxy (latency, bandwidth, packet-loss, slow-close).',
  {
    ...
    preset: z
      .string()
      .describe('Preset: latency | bandwidth | packet-loss | slow-close'),
  },
...
server.tool(
  ...
  async ({ id, targetUrl }) => {
    const params = targetUrl
      ...
      : '';
    return api(`/api/webhooks/events/${id}/replay${params}`, {
      method: 'POST',
    });
  },
);

server.tool('webhook_clear', 'Clear all stored webhook events.', {}, async () =>
  api('/api/webhooks/events', { method: 'DELETE' }),
);
...
server.tool(
  ...
  async ({ id }) =>
    api(`/api/webhooks/simulations/${id}/fire`, { method: 'POST' }),
);
...
server.tool(
  ...
  'Create a new user with role and workspace assignment.',
  {
    ...
    workspaceId: z
      .string()
      .optional()
      .describe("Workspace ID (default: 'default')"),
  },
...
server.tool(
  ...
  'List all environment templates (built-in + custom).',
  {
    builtIn: z
      .boolean()
      .optional()
      .describe('Include built-in templates (default true)'),
  },
  async ({ builtIn }) => {
...
server.tool(
  ...
  'Apply an environment template to generate mock files.',
  {
    ...
    variables: z
      .record(z.string())
      .describe('Variable values to substitute in the template'),
  },
  async ({ id, variables }) =>
    ...
    mocks: z
      .array(
        z.object({
          filename: z
            .string()
            .describe('Output filename (supports {{VAR}} substitution)'),
          content: z
            .string()
            .describe('WireMock JSON content (supports {{VAR}} substitution)'),
        }),
      )
  ...
  async ({
    ...
    api('/api/performance/baselines', {
      ...
      body: JSON.stringify({
        name,
        scriptId,
        metrics: { p95ResponseMs, p99ResponseMs, errorRate, requestsPerSec },
      }),
    }),
);
...
server.tool(
  ...
  async ({
    ...
    api(`/api/performance/baselines/${baselineId}/compare`, {
      method: 'POST',
      body: JSON.stringify({
        current: { p95ResponseMs, p99ResponseMs, errorRate, requestsPerSec },
      }),
    }),
);
...
server.tool('tracing_health', 'Check if Jaeger is available.', {}, async () =>
  api('/api/tracing/health'),
);
...
server.tool('storage_health', 'Check MinIO availability.', {}, async () =>
  api('/api/storage/health'),
);
...
server.tool(
  ...
  'Generate a mock coverage report by comparing an OpenAPI spec against existing WireMock mappings.',
  {
    ...
    specFile: z
      .string()
      .optional()
      .describe('Optional spec filename for display'),
  },
...
server.tool(
  ...
  'Get the current mock coverage percentage for an OpenAPI spec URL.',
  {
    specUrl: z
      .string()
      .describe(
        'URL of the OpenAPI spec (e.g. http://localhost:9090/api/docs-json)',
      ),
  },
  async ({ specUrl }) =>
    api(`/api/coverage/score?specUrl=${encodeURIComponent(specUrl)}`),
);
...
server.tool(
  ...
  'Create a stateful mock that responds with live data from a database query rendered through a Handlebars template.',
  {
    ...
    urlPath: z
      .string()
      .optional()
      .describe('Exact URL path to match, e.g. /api/users'),
    urlPattern: z.string().optional().describe('Regex URL pattern to match'),
    stateEngine: z
      .enum(['postgres', 'mysql', 'sqlite'])
      .describe('Database engine to query'),
    stateDatabase: z
      .string()
      .optional()
      .describe('Database name (optional, uses default if omitted)'),
    stateQuery: z
      .string()
      .describe('READ-ONLY SQL query to execute, e.g. SELECT * FROM users'),
    stateTemplate: z
      .string()
      .describe(
    ...
    responseStatus: z
      .number()
      .optional()
      .describe('HTTP response status (default: 200)'),
    fallbackBody: z
      .string()
      .optional()
      .describe('Static body returned if DB query fails'),
    queryTimeoutMs: z
      .number()
      .optional()
      .describe('Query timeout in milliseconds (default: 5000)'),
    cacheTtlSeconds: z
      .number()
      .optional()
      .describe('Cache TTL in seconds, 0 = disabled (default: 0)'),
  },
  async ({
    name,
    description,
...
server.tool(
  ...
  async ({
    ...
    return api(`/api/stateful/mocks/${id}`, {
      method: 'PUT',
      body: JSON.stringify(body),
    });
  },
...
server.tool(
  ...
  'Test a stateful mock against the current database state and return the rendered response.',
  {
    ...
    method: z
      .string()
      .optional()
      .describe('Override request method for testing'),
