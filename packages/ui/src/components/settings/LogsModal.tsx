...
export function LogsModal({ serviceId, serviceName, open, onClose, fetchLogs }: LogsModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1e1e2e] border border-white/10 rounded-2xl w-full max-w-3xl shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-auto p-4">
          {loading ? (
            <div className="text-text-secondary text-sm animate-pulse">Loading logs…</div>
          ) : (
