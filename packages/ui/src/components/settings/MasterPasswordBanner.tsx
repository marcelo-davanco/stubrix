...
export function MasterPasswordBanner({ status, onSetup, onUnlock, onLock }: MasterPasswordBannerProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'setup') {
        if (password.length < 8) { setError(t('settings.passwordMinLength')); return }
        if (password !== confirm) { setError(t('settings.passwordsDoNotMatch')); return }
        await onSetup(password)
        close()
      } else {
        const ok = await onUnlock(password)
        if (!ok) { setError(t('settings.incorrectPassword')); return }
        close()
      }
    } finally {
      setBusy(false)
  ...
  if (!status.configured) {
    banner = (
      <div className="flex items-center gap-3 bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4">
        ...
        <div className="flex-1 text-sm">
          <span className="text-yellow-200 font-medium">{t('settings.masterPasswordNotConfigured')}</span>
          <span className="text-yellow-200/70 ml-2">{t('settings.sensitivePlaintext')}</span>
        </div>
        <button
          type="button"
          onClick={openSetup}
    ...
    banner = (
      <div className="flex items-center gap-3 bg-green-500/10 border border-green-500/30 rounded-xl p-4">
        ...
        <div className="flex-1 text-sm">
          <span className="text-green-200 font-medium">{t('settings.encryptionActive')}</span>
          <span className="text-green-200/70 ml-2">{t('settings.sessionExpiresInMinutes', { count: mins })}</span>
        </div>
        <button
          type="button"
          onClick={() => void onLock()}
    ...
    banner = (
      <div className="flex items-center gap-3 bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
        ...
        <div className="flex-1 text-sm">
          <span className="text-blue-200 font-medium">{t('settings.encryptionConfigured')}</span>
          <span className="text-blue-200/70 ml-2">{t('settings.sessionLocked')}</span>
        </div>
        <button
          type="button"
          onClick={openUnlock}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#1e1e2e] border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <h2 className="text-base font-semibold mb-4">
              {mode === 'setup' ? t('settings.setupMasterPassword') : t('settings.unlockSession')}
            </h2>
            ...
            <div className="flex justify-end gap-2 mt-5">
              ...
              >
                {busy ? t('settings.processing') : mode === 'setup' ? t('settings.setup') : t('settings.unlock')}
              </button>
            </div>
          </div>
        </div>
