...
interface PreviewService {
  ...
  serviceName: string
  status: string
  changes: { key: string; action: string; currentValue?: string; importedValue: string; reason?: string }[]
}
...
export function ImportWizard({ open, onClose, onComplete }: ImportWizardProps) {
  ...
  const reset = () => {
    setStep(1); setFile(null); setIsEncrypted(false); setMasterPassword('')
    setConflictStrategy('skip'); setSelectedServices([]); setPreview(null)
  ...
  const handleFile = (f: File) => {
    ...
    if (!['json', 'yaml', 'yml'].includes(ext ?? '')) {
      setError('Accepts: .json, .yaml, .yml'); return
    }
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      if (isEncrypted && masterPassword) fd.append('masterPassword', masterPassword)

      const res = await fetch('/api/settings/import/preview', { method: 'POST', body: fd })
      if (!res.ok) {
      ...
      setSelectedServices(
        data.services
          .filter((s) => s.status !== 'unchanged' && s.status !== 'unknown-service')
          .map((s) => s.serviceId),
      )
      setStep(2)
    } catch (e) {
              ...
              >
                ...
                {file ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-text-secondary/60 mt-1">Accepts: .json, .yaml, .yml</p>
                  </>
                )}
              </div>
              <input
          ...
          {step === 2 && preview && (
            <>
              <div className="bg-white/5 rounded-xl p-3 text-sm space-y-1">
                <p><span className="text-text-secondary">File:</span> <span className="font-mono text-xs">{file?.name}</span></p>
                <p><span className="text-text-secondary">Services:</span> {preview.services.length} | <span className="text-text-secondary">Changes:</span> {preview.totalChanges}</p>
              </div>

              {preview.warnings.length > 0 && (
                <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3">
                  <p className="text-xs font-semibold text-yellow-400 mb-1">⚠ {preview.warnings.length} warning{preview.warnings.length > 1 ? 's' : ''}</p>
                  {preview.warnings.map((w, i) => <p key={i} className="text-xs text-yellow-300/80">• {w}</p>)}
                </div>
              )}

              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-text-secondary mb-2">Select services to import</p>
                <div className="space-y-1 max-h-40 overflow-y-auto">
                  {preview.services.map((svc) => {
                    const changeCount = svc.changes.filter((c) => c.action !== 'skip').length
                    const isDisabled = svc.status === 'unchanged' || svc.status === 'unknown-service'
                    return (
                      <label key={svc.serviceId} className={`flex items-center justify-between px-3 py-2 rounded-lg transition-colors ${isDisabled ? 'opacity-40' : 'cursor-pointer hover:bg-white/5'}`}>
                        ...
                        <span className="text-xs text-text-secondary">
                          {svc.status === 'unchanged' ? 'unchanged' : `${changeCount} change${changeCount !== 1 ? 's' : ''}`}
                        </span>
                      </label>
                    )
                  })}
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-text-secondary mb-2">Conflict Strategy</p>
                <ConflictStrategySelector value={conflictStrategy} onChange={setConflictStrategy} />
              </div>
          ...
          {step === 3 && result && (
            <div className="text-center py-4 space-y-4">
              ...
              <div className="text-sm text-text-secondary space-y-1.5 text-left max-w-xs mx-auto">
                <p className="flex items-center gap-2"><span className="text-green-400">✓</span> {result.servicesImported} services imported</p>
                <p className="flex items-center gap-2"><span className="text-green-400">✓</span> {result.configsUpdated} configs updated</p>
                <p className="flex items-center gap-2"><span className="text-green-400">✓</span> {result.configsCreated} configs created</p>
                <p className="flex items-center gap-2"><span className="text-text-secondary">⊘</span> {result.configsSkipped} configs skipped</p>
                {result.errors.length > 0 && (
                  <p className="flex items-center gap-2 text-red-400"><AlertTriangle size={14} /> {result.errors.length} error{result.errors.length > 1 ? 's' : ''}</p>
                )}
                {result.autoBackupId && (
                  <p className="text-xs text-text-secondary mt-2">Safety backup: <span className="font-mono">{result.autoBackupId}</span></p>
                )}
              </div>
            </div>
          )}
