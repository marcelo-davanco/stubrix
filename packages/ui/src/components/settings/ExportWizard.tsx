...
export function ExportWizard({ open, onClose }: ExportWizardProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = contentDisposition.match(/filename="([^"]+)"/)?.[1] ?? `stubrix-config.${format}`
      ...
      <div className="bg-[#1e1e2e] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="px-5 py-5 space-y-4">
            ...
            <>
              <div className="space-y-2">
                <label className="flex items-center gap-3 p-3 rounded-lg border border-white/10 cursor-pointer hover:border-white/20 transition-colors">
                  <input type="radio" checked={allServices} onChange={() => setAllServices(true)} className="accent-primary" />
                  <span className="text-sm">All services ({services.length})</span>
                </label>
                <label className="flex items-center gap-3 p-3 rounded-lg border border-white/10 cursor-pointer hover:border-white/20 transition-colors">
                  <input type="radio" checked={!allServices} onChange={() => setAllServices(false)} className="accent-primary" />
          ...
          {step === 2 && (
            <>
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-text-secondary mb-2">Format</p>
                <div className="flex gap-2">
                  {(['json', 'yaml'] as const).map((f) => (
                    <label key={f} className={`flex items-center gap-2 px-4 py-2 rounded-lg border cursor-pointer transition-colors ${format === f ? 'border-primary/50 bg-primary/10' : 'border-white/10 hover:border-white/20'}`}>
                      <input type="radio" checked={format === f} onChange={() => setFormat(f)} className="accent-primary" />
