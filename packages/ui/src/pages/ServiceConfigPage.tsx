...
export function ServiceConfigPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      showToast(
        'error',
        e instanceof Error ? e.message : t('serviceConfig.saveFailed'),
      );
    }
  };
  ...
  const handleResetAll = async () => {
    try {
      ...
      showToast(
        'error',
        e instanceof Error ? e.message : t('serviceConfig.resetFailed'),
      );
    }
  };
  ...
  const handleRollback = async (historyId: number) => {
    try {
      ...
      showToast(
        'error',
        e instanceof Error ? e.message : t('serviceConfig.rollbackFailed'),
      );
    }
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold">
              {service?.name ?? serviceId} {t('serviceConfig.configuration')}
            </h1>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Save size={14} />
              {saving
                ? t('serviceConfig.saving')
                : hasChanges
                  ? t('serviceConfig.saveCount', {
                      count: String(Object.keys(dirty).length),
                    })
                  : t('serviceConfig.save')}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#1e1e2e] border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl p-6">
            <h2 className="text-base font-semibold mb-2">
              {t('serviceConfig.resetConfirmTitle')}
            </h2>
            <p className="text-sm text-text-secondary mb-5">
              {t('serviceConfig.resetConfirmDesc')}
            </p>
            <div className="flex justify-end gap-2">
              ...
              >
                {t('serviceConfig.resetAllButton')}
              </button>
            </div>
          </div>
        </div>
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-xl shadow-lg text-sm font-medium transition-all ${toast.type === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}
        >
          {toast.msg}
        </div>
      )}
